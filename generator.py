...
def generate_dataset(num_measurements: int,
    ...
    if input_e1:
        ...
        # Create impulse response input directly on device
        impulse_response_input = torch.zeros(1, sequence_length, 1, device=device)
        impulse_response_input[0, 0, 0] = 1.0
...
def generate_teacher(teacher_rank: int,
    ...
    Optimized to generate directly on target device.
    """
    teacher_dim = teacher_rank if teacher_rank > 1 else 2
    
    # Generate A_teacher directly on device
    A_teacher = torch.zeros(teacher_dim, device=device)
    A_teacher[:teacher_rank] = torch.normal(mean=0.5, std=0.1, size=(teacher_rank,), device=device)
    
    # Only log scalar values to reduce CPU overhead
    logging.info(f"A_teacher[0] is {A_teacher[0].item()}")

    # Generate B_teacher directly on device
    ...
    # Generate C_teacher directly on device
