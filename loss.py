...
def get_train_and_gen_from_y(y: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
...
def sensing_loss(y_teacher: torch.Tensor,
                 y_student: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Calculate the sensing loss for the student model.
        """
        y_teacher_train, y_teacher_gen = get_train_and_gen_from_y(y_teacher)
        y_student_train, y_student_gen = get_train_and_gen_from_y(y_student)
...
def gnc_sensing_loss(y_teacher: torch.Tensor,
    ...
    return sensing_loss(y_student=y_student, y_teacher=y_teacher)


def gd_sensing_loss(y_pred: torch.Tensor,
                    y_teacher: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    """
    return sensing_loss(y_student=y_pred, y_teacher=y_teacher)