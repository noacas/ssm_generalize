...
def run_experiment(train_inputs_list, train_outputs_list, ext_inputs, ext_outputs, adaptive, student_state_dim, seeds, base_lr, eps, epochs, debug=False):
    ...
    for train_inputs, train_outputs, seed in zip(train_inputs_list, train_outputs_list, seeds):
        ...
        if debug:
            print(f"Train loss: {train_loss}, Ext. loss: {ext_loss}")
    print("-------------------------------------------------------------------------")
    print("-------------------------------------------------------------------------")
...
def beyond_theory_one(alpha_teacher, adaptive, student_state_dim, length, ext_length, eps, fixed_inputs, base_lr, epochs, num_baseline, num_special, with_baseline_in_poison, debug):
    ...
    if not fixed_inputs:
        ...
        train_inputs[0, :, :] = [[0.5798], [-2.0641], [-0.8111], [0.5352], [0]]
        train_inputs[1, :, :] = [[0.5], [-2], [-0.8], [0.5], [0]]
        train_inputs[2, :, :] = [[0.5], [-2], [-0.5], [0.5], [0]]
        train_inputs[3, :, :] = [[0.5], [0], [0], [0.5], [0]]
        train_outputs = [teacher(ipt) for ipt in train_inputs]
        train_losses, ext_losses = run_experiment(train_inputs, train_outputs, ext_inputs, ext_outputs, adaptive, student_state_dim, seeds, base_lr, eps, epochs, debug=debug)
        print("-------------------------------------------------------------------------")
        print("-------------------------------------------------------------------------")
        print("Summary:")
...
if __name__ == "__main__":
    ...
    beyond_theory_one(alpha_teacher, adaptive, student_state_dim, length, ext_length, eps, fixed_inputs, base_lr, epochs, num_baseline, num_special, with_baseline_in_poison, debug)